...
struct ArrayHash {
    size_t operator()(const std::array<int, 5>& a) const {
        ...
        for (int i = 1; i < 5; ++i) {
            seed ^= std::hash<int>{}(a[i]) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
        }
        return seed;
    }
};
...
public:
    ...
    int Dim;
    double Sigma;
    std::array<int, 5> string_head; // Range for the random walk
    std::vector<std::array<int, 5>> string;
    int length_string;
    int complete_length_string; // Total length of the random walk
    
    std::unordered_set<std::array<int, 5>, ArrayHash> visited; // Store visited positions
