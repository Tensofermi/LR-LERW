...
public:
    ...
    int Dim;
    double Sigma;
    std::array<int, 4> string_head; // Range for the random walk
    std::vector<std::array<int, 4>> string;
    int length_string;
    int complete_length_string; // Total length of the random walk
    
    std::unordered_set<std::array<int, 4>, ArrayHash> visited; // Store visited positions
